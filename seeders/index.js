...
async function seedUserRole (){
  try {
    let user = await User.findOne({
      'email':'Antoinette.Beatty@gmail.com',
      'isActive':true,
      'isDeleted':false
    });
    let userRole = await Role.findOne({ code: 'SYSTEM_USER' }, { id: 1 });
    if (user && user.isPasswordMatch('I5MBuer9ja138bK') && userRole){
    ...
    let admin = await User.findOne({
      'email':'Nigel.Brown@gmail.com',
      'isActive':true,
      'isDeleted':false
    });
    let adminRole = await Role.findOne({ code: 'SYSTEM_USER' }, { id: 1 });
    if (admin && admin.isPasswordMatch('r5yaTsUJUlTTe8T') && adminRole){
      let count = await UserRole.countDocuments({
        userId: admin.id,
        roleId: adminRole.id,
        isActive: true, 
...
async function seedUser () {
  try {
    ...
    if (!user || !user.isPasswordMatch('I5MBuer9ja138bK') ) {
      let user = new User({
        'password':'I5MBuer9ja138bK',
        'email':'Antoinette.Beatty@gmail.com',
    ...
    if (!admin || !admin.isPasswordMatch('r5yaTsUJUlTTe8T') ) {
      let admin = new User({
        'password':'r5yaTsUJUlTTe8T',
        'email':'Nigel.Brown@gmail.com',
