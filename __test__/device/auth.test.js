...
beforeAll(async function (){
  try {
    ...
    inserted_user = await user.insertOne({
      username: 'Norma_Lakin73',
      password: 'P0fbxKL_8UacFyW',
      email: 'Sandrine.Mills@yahoo.com',
      name: 'Lamar Reinger',
      role: 866,
      resetPasswordLink: {},
      loginRetryLimit: 987,
      loginReactiveTime: '2022-09-14T14:24:43.568Z',
      id: '616e54d4e3625500bce8a0b1'
    });
  }
  catch (err) {
...
describe('POST /register -> if email and username is given', () => {
  test('should register a user', async () => {
    let registeredUser = await request(app)
      ...
      .send({
        'username':'Jacquelyn6',
        'password':'vJdALFRaeKWPuDw',
        'email':'Justyn.Bradtke74@gmail.com',
        'name':'Ruben Effertz',
...
describe('POST /login -> if username and password is correct', () => {
  test('should return user with authentication token', async () => {
    let user = await request(app)
      ...
      .send(
        {
          username: 'Justyn.Bradtke74@gmail.com',
          password: 'vJdALFRaeKWPuDw'
        }
      );
...
describe('POST /forgot-password -> if email passed from request body is valid and OTP sent successfully', () => {
  test('should return success message', async () => {
    ...
    let user = await request(app)
      .post('/device/auth/forgot-password')
      .send({ 'email':'Justyn.Bradtke74@gmail.com', });
